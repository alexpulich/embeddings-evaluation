"""
Note: adapted from https://github.com/kudkudak/word-embeddings-benchmarks
"""
...
def load_embedding(fname, format="word2vec_bin", normalize=True,
    ...
    if format == 'word2vec_bin':
        embeddings = Embedding.from_word2vec(fname, binary=True)
    elif format == 'word2vec':
        embeddings = Embedding.from_word2vec(fname, binary=False)
    elif format == 'glove':
        embeddings = Embedding.from_glove(fname, **load_kwargs)
    elif format == 'dict':
        embeddings = Embedding.from_dict(pickle.load(open(fname, "rb"), encoding='latin1'))


    if normalize:
        embeddings.normalize_words(inplace=True)

    if lower or clean_words:
        embeddings.standardize_words(lower=lower, clean_words=clean_words, inplace=True)
